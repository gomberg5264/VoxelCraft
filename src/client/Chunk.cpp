    ...
    for (unsigned x = 0; x < chunkDimension.x; x++)
    {
        for (unsigned y = 0; y < chunkDimension.y; y++)
        {
            for (unsigned z = 0; z < chunkDimension.z; z++)
            {
                ...
                const float perlin = glm::perlin(glm::fvec2(
                    float(int(x) + m_pos.x) / chunkDimension.x, 
                    float(int(z) + m_pos.z) / chunkDimension.z) * flatFactor);
                assert(perlin >= -1.f && "Why is perlin negative???");
                // perlin [0,1]
                const unsigned height = static_cast<unsigned>((perlin + 1.f) / 2.f) * (chunkDimension.y * 0.5f) + (chunkDimension.y * 0.5f);
                glm::fvec3 pos{ x,y,z };
                BlockType type;
